...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      val inner_acc = if(p(elem))  acc incl elem;  else acc
      val left_acc = left.filterAcc(p,inner_acc)
      val right_acc = right.filterAcc(p,left_acc)
      right_acc
    }
